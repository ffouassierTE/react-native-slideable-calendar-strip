...
// const week_localized = isChinese ? WEEK_en : WEEK_fr;
const weekStartsOnMinnor = 1 % 7;
const weekTransformed = [
  ...WEEK_fr.slice(weekStartsOnMinnor),
  ...WEEK_fr.slice(0, weekStartsOnMinnor),
];
...
class DateItem extends PureComponent {
  render() {
    ...
    const highlightBgColor = '#0592FF';
    const normalBgColor = 'white';
    const highlightTextColor = '#fff';
    const normalTextColor = '#494a4b';
      ...
      <View style={styles.itemContainer}>
        ...
        >
          <View style={[
            ...
            <Text style={[
              styles.itemDateText,
              {color: highlight ? highlightTextColor : normalTextColor}
            ]}>{solar}</Text>

            {/* {showLunar &&
              <Text style={[
                styles.itemLunarText,
                {color: highlight ? highlightTextColor : 'gray'}
              ]}>{lunar}</Text>
            } */}
          ...
          renderItem={({item, index}) => {
            return (
              <View>
                ...
                }} key={weekTransformed[index]}>
                  <Text style={{
                    color: '#494a4b',
                    fontSize: scale(12),
                  }}>{weekTransformed[index]}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  //   position: 'absolute', top: scale(5), right: scale(50),
